...
export function SyncBizPlayerUnified() {
  const {
    currentSource,
    currentPlayUrl,
    isEmbedded,
  ...
  const hasPrevNext = sources.length > 1;

  if (!currentSource) {
    return (
      <div className="rounded-2xl border border-slate-800/80 bg-slate-950/60 p-6 text-center">
        <p className="text-sm text-slate-500">No source selected. Click Play on any source to start.</p>
      </div>
    );
  }

  const cover = currentSource.cover;
  ...
  return (
    <div className="rounded-2xl border border-slate-800/80 bg-gradient-to-b from-slate-900/95 to-slate-950/95 p-6 shadow-[0_8px_32px_rgba(0,0,0,0.4)]">
      {/* 1. Cover + title hierarchy */}
      <div className="flex flex-col items-center gap-5 lg:flex-row lg:items-center">
        <div className="flex-shrink-0">
          <div className="aspect-square w-36 overflow-hidden rounded-2xl bg-slate-900 shadow-[0_4px_20px_rgba(0,0,0,0.4)] ring-1 ring-slate-800/80 sm:w-40">
            {cover ? (
        ...
        <div className="min-w-0 flex-1 text-center lg:text-left">
          <h2 className="truncate text-xl font-semibold text-slate-100">{currentSource.title}</h2>
          {currentSource.genre && <p className="mt-1 text-sm font-medium uppercase tracking-wider text-slate-500">{currentSource.genre}</p>}
          <p
          ...
          >
            <span className={`h-2 w-2 rounded-full ${status === "playing" ? "bg-emerald-400 animate-pulse" : status === "paused" ? "bg-amber-400" : "bg-slate-500"}`} />
            {status}
          </p>
        </div>
      </div>

      {/* 2. Transport controls – PLAY central and prominent */}
      <div className="mt-8 flex flex-wrap items-center justify-center gap-4" role="group" aria-label="Player controls">
        ...
        <ActionButtonPlay
          onClick={() => currentSource && playSource(currentSource)}
          size="xl"
          title="Play"
          aria-label="Play"
        />
      ...
      {!isEmbedded && <p className="mt-4 text-center text-sm text-slate-500">Local playback. Use system player or Winamp.</p>}
    </div>
  );
}
