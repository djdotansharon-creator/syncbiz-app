...
- **`components/audio-player.tsx` (~4.4k lines — NEVER read whole)**
  ...
  - **Video dock** (41dd88d): NO buttons — the hidden YT A/B prewarm wrapper docks `absolute right-12 top-1/2 -translate-y-1/2 w-[19rem]` inside the deck whenever `isYouTube && playing/paused` (`videoDocked` IIFE near the prewarm render; NOTE `fixed` resolves to a transformed ancestor here — use absolute). Bottom fade overlay = card-fog language. Iframes NEVER remount/reparent (audio!); `videoActiveDeck` state mirrors `ytActiveDeckRef` via always-on 500ms change-only poll → visual crossfade; `pointer-events-none` so YT UI can't pause the business.
