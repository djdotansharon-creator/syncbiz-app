...
- **Search bar**: `components/library-input-area.tsx` — single slim bar (h-8): URL ingest | divider | search; quiet mic/AI buttons. Results dropdown redesigned Spotify-scale (fb6bf6c): shared consts `RESULT_ROW/THUMB/TITLE/META/PLAY_BTN/GHOST_BTN` + `ResultPlatformLogo` (search those symbols); five sections (My Music / library / catalog / radio / YouTube); no letter chips — inline platform logos.
