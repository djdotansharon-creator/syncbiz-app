...
- **`components/audio-player.tsx` (~4.4k lines — NEVER read whole)**
  ...
  - **Video panel** (583969c): `videoPanelOpen` state + "Video"/X buttons; the hidden YT A/B prewarm wrapper docks `absolute bottom-3 right-3` INSIDE the deck when open (NOTE: `fixed` resolves to a transformed ancestor here — use absolute). Iframes NEVER remount/reparent (audio!); active-deck opacity swap via 400ms poll of `ytActiveDeckRef` while open; `pointer-events-none` on video so YT UI can't pause the business.
