...
import { sanitizeRegistrationIntent } from "./syncbiz-device-model.js";
...
function primaryReservedInGrace(userId, branchId) {
    const key = branchKey(userId, branchId);
    const primaryId = primaryMasterByBranch.get(key);
    if (!primaryId)
        return null;
    if (devices.get(primaryId)?.ws.readyState === 1)
        return null;
    const disconnectedAt = masterDisconnectedAt.get(key);
    if (!disconnectedAt)
        return null;
...
function streamerBlocksFallbackMaster(userId, branchId, requesterDeviceId) {
    if (getActiveStreamerMaster(userId, branchId))
        return true;
...
function logLeaseEvent(event, data) {
...
function broadcastLibraryUpdated(userId, branchId, entityType, action, entityId) {
    const msg = { type: "LIBRARY_UPDATED", branchId, entityType, action, entityId };
    ...
    ws.on("message", (data) => {
            ...
            if (role === "device" && validation.deviceId) {
                ...
                if (isMobile) {
                    ...
                    if (streamerBlocksFallbackMaster(userId, branchId, deviceId)) {
                        ...
                        const currentMaster = currentMasterId && currentMasterId !== deviceId ? devices.get(currentMasterId) : undefined;
                        const someoneElseAlreadyMaster = !!currentMaster && currentMaster.ws.readyState === 1 && currentMaster.mode === "MASTER";
        ...
        if (msg.type === "SET_MASTER" && role === "device" && deviceId) {
            ...
            if (requesterIsDesktop && streamerBlocksFallbackMaster(userId, branchId, deviceId)) {
                ...
                if (ws.readyState === 1) {
                    ws.send(JSON.stringify({ type: "ERROR", message: "Streamer has branch audio priority" }));
                }
                return;
            }
            const currentMasterId = masterByBranch.get(key);
            const currentMaster = currentMasterId && currentMasterId !== deviceId ? devices.get(currentMasterId) : undefined;
            const dedicatedActiveMaster = !!currentMaster &&
            ...
            if (!requesterIsDedicated && dedicatedActiveMaster) {
                ...
                if (ws.readyState === 1) {
                    ws.send(JSON.stringify({ type: "ERROR", message: "Dedicated player is currently MASTER" }));
                }
                return;
            }
            demoteOtherMasters(userId, branchId, deviceId, deviceId);
