...
exports.classifyResolveFoundation = classifyResolveFoundation;
const playlist_utils_1 = require("./playlist-utils");
const browserPrefer = {
...
function youtubeSingleVideoSignals(url) {
    const u = url.trim();
    if (/youtube\.com\/shorts\/[^/?\s]+/i.test(u))
        return true;
    if ((0, playlist_utils_1.getYouTubePlaylistId)(u))
        return false;
    if (/(?:youtube\.com\/watch\?v=|youtu\.be\/|youtube\.com\/embed\/)([^&\s?/]+)/i.test(u))
        return true;
    return false;
}
function spotifyPathKind(url) {
    const u = url.toLowerCase();
...
function soundcloudPathKind(url) {
    const u = url.toLowerCase();
 ...
 */
function classifyResolveFoundation(ctx) {
    ...
    if (inferredType === "youtube" || isYouTubeHost(rawUrl)) {
        const listId = (0, playlist_utils_1.getYouTubePlaylistId)(rawUrl);
        const mixLike = (0, playlist_utils_1.isYouTubeMixUrl)(rawUrl);
